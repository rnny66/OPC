...
**Implementation plan:** [phase-1-foundation-impl.md](phase-1-foundation-impl.md)
**Status:** All 12 tasks complete. 21 unit tests + 3 pgTAP tests passing. Build succeeds.
...
- [x] Static site works from `site/` folder with no broken links or assets
- [x] Next.js app runs at `localhost:3000`
- [x] Can sign up with email/password, receive confirmation email
- [ ] Can sign up with Google OAuth (needs OAuth config in Supabase dashboard)
- [ ] Email verification flow works end-to-end (needs email template config)
- [x] Middleware blocks unauthenticated users from protected routes
- [x] Middleware blocks non-organizers from `/organizer/*` routes
- [x] `profiles` row is auto-created on signup
- [x] RLS policies prevent unauthorized access (applied via Supabase MCP)
